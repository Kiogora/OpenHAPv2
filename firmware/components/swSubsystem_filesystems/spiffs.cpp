...
softwareSubsystem::filesystems::spiffs::spiffs(supportedMemories setupOnThisMemory, std::string mountPoint, size_t maxFiles, bool formatIfMountFails):
thisMemory{setupOnThisMemory}
{
    if(setupOnThisMemory == supportedMemories::internalSPIFlash)
    {
...
esp_err_t softwareSubsystem::filesystems::spiffs::getBytesAvailable(size_t& used, size_t& total)
{
